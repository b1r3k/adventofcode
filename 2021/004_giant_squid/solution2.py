...
class Board:
    ...
    def is_winning_row(self, row_idx):
        ...
        self.winning = all(marks)
        return self.winning

    def is_winning_column(self, col_idx):
        marks = self._get_column_marks(col_idx)
        self.winning = all(marks)
        return self.winning
        ...
        for board_idx, board in enumerate(nice_boards):
            for has_number in board.mark_number(winning_number):
                ...
                if has_winning_column or has_winning_row:
                    ...
                    yield board_idx, winning_number * unmarked_sum


def meta_solution(winning_numbers, boards):
    scores = {board_idx: 0 for board_idx in range(0, len(boards))}
    for board, score in solution(winning_numbers, boards):
        # print(f'Board {board} is a winner! Score: {score} others: {scores.values()}')
        if scores[board] == 0:
            scores[board] = score
...
if __name__ == '__main__':
    with open('test_input.txt', 'r') as f:
        data = read_input(f)
    test_data_solution = meta_solution(*data)
    assert test_data_solution == 1924, f'invalid response ({test_data_solution}) to test data'
    data = read_input(sys.stdin)
    print(meta_solution(*data))
